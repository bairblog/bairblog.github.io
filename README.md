...
To upload images to the server, see instructions [here](#push-to-the-server) and [here](#setting-permissions).
...
- ... that production branch has specific `baseurls` and `urls` that shouldn't
  change in `_config.yml`. This will be different from the master branch, so watch out!  The
  master branch uses:
...
```
scp -r _site/* seita@login.eecs.berkeley.edu:/project/eecs/bair/www-bair/blog/
```
...
This requires changing the **group** to be `bair-www` and the permissions set to
...
(By the way, you obviously need to be a member of the `bair-www` group to do
