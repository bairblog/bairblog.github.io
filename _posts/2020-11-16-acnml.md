...
classify automobiles, where the all the class 1 training examples are sedans
and class 2 examples are large buses.
...
<br />
<i>
Figure 3: Query point shown in purple. Both $\hat \theta_{\text{train}}$ and
...
<br />
<i>
Figure 4: Query point shown in purple. All parameters are forced to classify
the query point as class 0 since it is in the middle of the class 0 training
points.
</i>
</p>
...
prior term, resulting in conditional normalized maximum a posteriori (CNMAP)
[cite Kakade GP paper] instead. Instead of computing maximum likelihood
...
our method, *amortized monditional normalized maximum likelihood* (ACNML), we
...
## Conditional NMAP and Bayesian Posteriors
...
We first examine ACNML’s predictions using reliability diagrams [cite], which
...
We quantitatively measure calibration using the Expected Calibration Error
metric (cite), which uses the same buckets as the reliability diagrams and
