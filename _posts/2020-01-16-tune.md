...
3. Provides reference implementations of many state-of-the-art hyperparameter search algorithms.
...
**Actor-based Training**: Many techniques for hyperparameter optimization
...
*Ray Actor API*: Ray [provides an API][7] to create an “actor” from a Python
...
**Fault Tolerance**: Cloud providers often offer “preemptible instances” (i.e.,
...
*How does it work?* If a node is lost while a training job is still executing
...
head_node: {InstanceType: c4.2xlarge, ImageId: ami-0b294f219d14e6a82}
worker_nodes: {InstanceType: c4.2xlarge, ImageId: ami-0b294f219d14e6a82}
...
# Starts a cluster, uploads and runs a script on the specified cluster, then shuts down
# the cluster.
$ ray submit CLUSTER.yaml --start --stop tune_experiment.py --args="--address=auto"
```
...
class TrainMNIST(tune.Trainable):
    def _setup(self, config):
        ...
        self.model = ConvNet()
        self.optimizer = optim.SGD(self.model.parameters(), lr=config.get("lr", 0.01))
...
tuning, used in many other popular research projects, ranging from
[Population-based Data Augmentation][22], to [Hyperparameter Tuning for
AllenNLP][23], to [AutoML in AnalyticsZoo][24].
