...
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575499318030_fully_off_policy_teaser.png" width="">
</p>
...
<p style="text-align:center;">
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575315719190_figure1_problem_exists.png" width="">
<br />
<i>
...
$$Q^* = \mathcal{T}^* Q^*  (\mathcal{T}^* \hat{Q})(s, a) := R(s, a) + \gamma \mathbb{E}_{T(s'|s,a)}[\max_{a'}\hat{Q}(s', a')]$$
...
<p style="text-align:center;">
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575316706286_problem_Q-values.png" width="">
<br />
<i>
...
<p style="text-align:center;">
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575317259331_distinction_methods.png" width="">
<br />
<i>
...
distance between actions as a measure of support divergence ($k$ is any RBF
kernel):
...
Some works, for example, [KL-Control](https://arxiv.org/abs/1907.00456),
[BRAC](https://arxiv.org/abs/1911.11361),
[G-Learning](https://arxiv.org/abs/1512.08562), argue that using a
distribution-matching constraint might suffice in such fully off-policy RL
problems. In this section, we take a slight detour towards deeply analyzing
this choice. In particular, we provide an instances of a MDP where
distribution-matching constraint might lead to arbitrarily suboptimal behavior
while support matching does not in principle suffer from this issue.

Consider the 1D-lineworld MDP shown in Figure 4 below. Two actions (left and
right) are available to the agent at each state. The agent is tasked with
starting from state $$S$$ and going to the goal state $$G$$, and the
corresponding reward values are shown in Figure 4, left. The agent is only
allowed to learn from behavior data from a policy that performs actions with
described likelihoods, and in particular, takes the suboptimal action at states
between $$S$$ and $$G$$ with a high likelihood of $$0.9$$, however both actions
$$\leftarrow$$ and $$\rightarrow$$ are in-distribution at all these states. In
Figure 4c, we show that the learned policy with a distribution-matching
constraint can be arbitrarily suboptimal, in fact, the probability of ending up
at the goal $$G$$ by rolling out this policy will be very small, and will tend
to 0 as the environment is made larger. However, in Figure 4d, we show that a
support constraint can recover an optimal policy with probability $$1$$.
...
<br />
<i>
Figure 5: Performance on (1) medium-quality dataset: BEAR outperforms the best performing baseline.
</i>
</p>

<p style="text-align:center;">
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575194824925_Screen+Shot+2019-12-01+at+2.06.48+AM.png" width="">
<br />
<i>
Figure 6: (3) Expert data: BEAR recovers the performance in the expert dataset, and performs similarly to other methods such as BC.
</i>
</p>

<p style="text-align:center;">
<img src="https://paper-attachments.dropbox.com/s_03D8A88577B961181603AE5EDBD4A511CD8E828E7651B8AA640A61950DAB9783_1575194836848_Screen+Shot+2019-12-01+at+2.05.38+AM.png" width="">
<br />
<i>
Figure 7: (2) Random data: BEAR recovers better than dataset performance, and is close to the best performing algorithm (Naive RL).
</i>
</p>
