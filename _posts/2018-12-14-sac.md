...
author:             <a href="https://people.eecs.berkeley.edu/~haarnoja/">Tuomas Haarnoja</a><br>
...
- **Off-Policyness**. An algorithm is off-policy if we can reuse data collected
...
- **Stop / Resume Training**. When working with real hardware, whatever can go
  ...
  stream.
- **Robot Operating System**. Many robotic systems have a ROS interface. We
  provide a tutorial where we interface a simulator through ROS with our
  toolbox.
...
Soft actor-critic (SAC) is an off-policy model-free deep RL algorithm that is
well aligned with these requirements. In particular, we show that it is sample
efficient enough to solve real-world robot tasks in only a handful of hours,
robust to hyperparameters and works on a variety of simulated environments with
...
$$J(\pi) = \mathbb{E}_{(\pi) \sim \rho_\pi} \left[ {\sum_{t} r(\mathbf{s}_t, \mathbf{a}_t)
...
actor-critic to a technical report. In this post, we will view the objective as
...
We have included more benchmark results in the accompanying technical report.
...
Our first real-world tasks involve the Minitaur robot, a small-scale quadruped
...
inset at the bottom right. The robot must rotate the valve into the correct
position, with the colored part of the valve facing directly to the right, from
any random starting position. The initial position of the valve is reset
...
challenges and the physical difficulty of rotating the valve with such a complex
robotic hand.
...
<p style="text-align:center;">
    ...
    <br>
<i>
Rotating a valve with Dynamixel Claw  (UC Berkeley, Kristian Hartikainen, Vikash
Kumar, Henry Zhu, Abhishek Gupta, Tuomas Haarnoja, and Sergey Levine).
</i>
</p>
...
biggest challenge is to get the studs perfectly aligned before exerting a
...
Soft actor-critic solves all of these tasks extremely quickly: the Minitaur
locomotion and the block-stacking tasks take both 2 hours, and the valve-turning
...
[4]:https://bair.berkeley.edu/static/blog/dex_manip/hand_valve_rotation.gif
