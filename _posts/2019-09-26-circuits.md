...
<p style="text-align:center;">
    <img src="https://bair.berkeley.edu/static/blog/circuits/1.png"
    width="600">
    <br>
</p>
...
The intent of analog IC design is to build physical manufacturable circuits
that process electrical signals in the analog domain, despite all sorts of
noise sources that might affect the fidelity of signals. Usually analog circuit
...
[RL][3]). We are mostly interested in approaches that find optimal sizing for a
given topology to satisfy a collection of metrics (a constraint satisfaction
problem). Some people have tried different mathematical formalizations to model
the behavior of circuits and turned the problem into analytical optimizations
with fast turn over (for example by modeling their objective as a form of
analytical function of inputs). However, as was mentioned earlier, today even
schematic simulations can differ from their layout sims. So those ancient
approaches lost their attractiveness very early on due to this important
drawback. So some people tried to use simulations and modeled the problem as a
...
On a side note, population based black-box optimization algorithms achieve
pretty good performance. For example, [this paper][4] shows a setting where RL
agents are trained in a parallelized fashion using scalable evolutionary
algorithms.  The problem is that they are insanely sample inefficient (albeit
...
simulation are not highly parallelizable and each process has its own license
requirement. In this work we have proposed a new way to make them more sample
efficient.
...
are larger than let’s say $A_0$ and $W_0$, respectively. We can formulate the
problem as minimizing a scalar cost function equal to the sum of errors
relative to the required spec, more concretely, in this example:
...
And we get the $A(x)$ and $BW(x)$ parameters after simulation.

To state in a more general form:
...
algorithms (great intro found [here][5]). In fact we used [deap][6] to
implement some baseline version of genetic algorithm. The problem here is that
...
accurate (or almost accurate) we waste fewer simulations. On the other hand, we
could also either approve samples that do not show high quality after
simulation or we could reject designs that should have been accepted.
...
First potential candidate is that we can have a regression model to predict the
cost value and then use this predicted value to determine whether or not to
simulate a design. The cost function that the network tries to approximate can
be non-convex and ill-conditioned. Thus, from a limited number of samples it is
very unlikely that it would generalize well to unseen data. Moreover, the cost
function captures too much information from a single scalar number, so it would
be hard to train, given a small number of training points and expect it
generalize well.
...
simulator used in population based optimization. This model takes in two sets
...
in spec1 or spec2 and so on.

So during inference the parameters of the query design are going to be fed to
Design B and the reference design to Design A and we predict how the query
design will do compared to the old design. Then we use some heuristic to decide
whether to simulate or ignore the new design.
...
and B the probability should become 0.2 by construction of the network. For
...
The reason that the rearrangement happens is exactly this, the code below shows
...
To train the network, we construct all DA and DB permutations from the buffer
of previously simulated designs and label their comparison in each metric. We
then update network parameters with Adam optimizer, using sum of cross-entropy
loss for all metrics.
...
track of which metrics we improved so far and what collection of metrics affect
the cost objective the most. For more info on details please refer to the
paper, and the code.
...
bigger size and longer simulations - to see if we this method can be applied to
long post-layout simulations of designs which are more complex. In this
particular example we also showed that we can even design circuits we very high
