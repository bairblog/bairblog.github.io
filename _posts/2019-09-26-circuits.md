...
<img src="https://bair.berkeley.edu/static/blog/circuits/bair_blog_post.svg" width="600">
<br>
</p>
...
$BW_0$. We want to find “optimum” sizes for the components of the circuit such
that it satisfy these performance constraints.
We can formulate the problem as minimizing a scalar cost function equal to the
sum of errors relative to the required spec, more concretely, in this example:
...
cost function that the network tries to approximate can be non-convex and
ill-conditioned. Thus, from a limited number of samples it is very unlikely
...
to train, given a small number of training points and expect it generalize
well.
