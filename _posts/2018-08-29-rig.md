...
it to be. To specify a new task, a user needs to simply provide a goal image. We
...
$(s, a, s’)$. However, if we assume that we have access to the reward function
$r(s, g)$, we can retroactively relabeled goals and recompute rewards, meaning
that we can go back and relabel a goal. So, we can modify this training
procedure like so:
</p>
...
) operate on ground truth state informatios <v></v>n (e.g., the Cartesian
...
model, which in our case in a variational autoencoder. This generative model
...
into the prior, we simply sample from our latent variable prior to obtain
...
to 84x84 RGB images, without access to joint angles or object positions. The
...
will inform the policy learn more about what it can and cannot reach. Another
