...
<i>This post is cross-listed at the Stanford AI blog**(TODO LINK)** and the
Carnegie Mellon blog**(TODO LINK)**.</i>
...
perception tasks they have already been successfully applied to.
...
Instead, we pre-train [visual dynamics
...
quality, but even large models with 500M parameters (middle row in the videos
below) are still quite blurry. This suggests ample room for improvement, and we
hope that the development of newer more powerful models will translate to
better control performance in the future.
...
parameter model (right-most) generates significantly blurrier predictions than
a large model with 500M parameters (center).
</i>
</p>
