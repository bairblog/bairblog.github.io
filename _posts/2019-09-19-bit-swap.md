---
layout:             post
title:              "A Deep Learning Approach to Compression"
...
advance this line of modern compression ideas. We also release a [demo][3] and a
pre-trained model for Bit-Swap image compression and decompression on your own
image. See the end the post for a talk that covers how bits-back coding and
Bit-Swap works.
...
ELBO on average, in addition to an overhead that only occurs at initialization.
This overhead becomes insignificant when compressing long sequences of
datapoints at once.
...
2. Bit-Swap is still able to compress close to the ELBO on average, in addition
...
breaking the images up in 32 x 32 pixel blocks. The results are shown below. We
believe results can be further improved by using bigger pixel patches and more
sophisticated model optimization.
